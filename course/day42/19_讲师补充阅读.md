...
ApprovalWorkflowGraph 所体现的「决策-执行-观察」循环，与 ReAct 论文（Yao et al.）的思路一致：让模型在自然语言里穿插推理与行动。
...
先把 `approval` 设计对，比先把决策准确率调高更重要——没有可观测性，线上问题排查会非常痛苦。
...
2. `agent/approval_workflow_graph.py`
3. Day 43 预习
