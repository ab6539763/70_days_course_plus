...
**15:20–16:00** 第七节：validation-config API  
...
```bash
curl -s localhost:8000/api/knowledge/validation-config | jq .
curl -s -X PUT localhost:8000/api/knowledge/validation-config \
