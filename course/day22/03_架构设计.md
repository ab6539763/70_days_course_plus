...
**模块**：`frontend/*` + `nexus-agent-platform/src/day22/*`  
**需求**：ZL-NA-REQ-022
...
graph TB
    ...
    subgraph day23[Day23 待接]
        API[src/api/chat.py]
    end
    subgraph backend[Day21 已有]
        CO[ChatOrchestrator.handle_message]
    end
    HTML --> APP
    CSS --> HTML
    MOCK --> APP
    APP -->|useMock true| MOCK
    APP -->|useMock false| API
    API --> CO
    MOCK -.契约对齐.-> CO
```
...
| 文件 | 层级 | 职责 |
|------|------|------|
| index.html | 结构 | 语义标签、元素 id、脚本引用 |
| style.css | 表现 | 布局、气泡、标签、动画 |
| app.js | 行为 | 事件、DOM 渲染、parseReply |
| mock.js | 数据 | FAQ/路由规则、API 占位 |
| frontend_audit.py | 质量 | 静态结构审计 |
| mock_bridge_demo.py | 对照 | 后端 vs 前端契约 |

## 3. app.js 模块边界

采用 IIFE 封装，不导出全局函数（除 mock.js 的 NexusMock）。内部函数：

- `scrollToBottom()` — 消息区滚动  
- `setLoading(on)` — UI 状态机  
- `parseReply(reply)` — 契约解析  
- `appendMessage(role, content, meta)` — DOM 工厂  
- `dispatchMessage(text)` — Mock/API 分发  
- `handleSubmit(event)` — 表单主路径  

## 4. mock.js 设计模式

**规则表驱动**：`FAQ_RULES` 与 `ROUTE_RULES` 数组，每项 `{ test, reply/template/body }`。优于巨型 if-else，便于学员扩展作业。

**全局单例**：`NexusMock` 挂载 `window`，供 `app.js` 检测 `mock.useMock`。

## 5. 与 ChatOrchestrator 契约

```python
# orchestrator.py 核心逻辑（Day 21）
def handle_message(self, user_text: str) -> str:
    # 1. FAQ 直答
    if match and match.score >= threshold:
        return f"[FAQ 直答·{match.score:.0%}] {answer}"
    # 2. auto_route chat_turn
    return self.assistant.chat_turn(user_text)
```

前端 Mock 不必复现 score 计算，但**前缀格式**必须一致，供 `parseReply` 与标签 CSS 使用。

## 6. 测试架构

```mermaid
graph LR
    TF[test_frontend.py] --> AUD[audit_frontend]
    TF --> FS[文件系统断言]
    TF --> STR[字符串断言]
    AUD --> CONST[day22/constants.py]
```

十二测试不启动浏览器（无 Playwright），采用**静态源码审计**，适合 CI 快速反馈。E2E 可作为 Day 24 扩展。
...
浏览器 http://localhost:8080
  ↓ (Day 23+)
nginx 反代 FastAPI + 静态文件
```

周航规划：Day 24 将 `frontend/` 与 API 同域部署，消除 CORS 摩擦。

## 8. 扩展点

| 扩展 | 位置 | 说明 |
|------|------|------|
| 新 FAQ 模式 | mock.js FAQ_RULES | 作业 F |
| 新路由 template | mock.js ROUTE_RULES | 与 IntentRouter 对齐 |
| 暗色主题 | style.css :root | 选做 |
| 真实 API | mock.js useMock | Day 23 |

架构文档完。详见 [11_静态聊天页详解.md](11_静态聊天页详解.md)。
