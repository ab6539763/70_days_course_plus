...
`frontend_audit.py` 预期输出：

```
==================================================
  Day 22 frontend 结构审计
==================================================
  目录: .../frontend
...
| 问句 | 观察 |
|------|------|
| 投资有风险吗 | 绿色 FAQ 直答标签，正文含风险揭示 |
| 帮我总结要点 | 紫色路由 · doc_summary |
| 你好 | 无标签，Meta「Mock 默认」 |
...
```javascript
if (reply.startsWith("[系统]")) {
  return { kind: "system", text: reply.slice(4).trim() };
}
```

测试用例示例：`[系统] 维护中` → system；`[FAQ 直答·70%] x` → faq；`[路由: a] b` → route；`plain` → bot；空串 → bot。
...
```javascript
{ test: (t) => /赎回|到账/.test(t), reply: "[FAQ 直答·71%] 赎回 T+1 到账...", meta: "FAQ 直答" }
```

ROUTE 新增 onboarding 不与后端冲突，属前端 Mock 扩展；Day 23 需在 IntentRouter 才有真实对应。

---

## 作业 E 参考答案

`--brand: #0d9488` 与白字对比度约 4.5:1 以上，可读。用户气泡白字 on 青色背景需验证 WCAG。
...
// Response 200
{ "reply": "[FAQ 直答·68%] ...", "meta": "FAQ 直答", "kind": "faq" }
...
- 作业 B 三张图缺一不可  
- 作业 D 须说明新增规则测试方式  
- 抄袭 mock.js 无 changelog 扣 10 分  

---

## 智链科技 Day 22 作业讲评补充（培训部）

### 高频错误 1：script 顺序颠倒

先 app 后 mock 导致 `NexusMock` undefined。讲评时演示 Console 报错。

### 高频错误 2：id 拼写

`message` vs `messages` 一字之差审计失败。强调对照 constants.py。

### 高频错误 3：忘记 trim

空消息仍发送。应依赖 required + JS trim 双保险。

### 讲评示范问句

讲师现场输入「年化收益率」「合规审阅」「客服电话」覆盖 FAQ 与 ROUTE 边界。

### 与竞赛衔接

作业 C parseReply 与 21_ 知识竞赛第 7 题同源，讲评后可抢答。
