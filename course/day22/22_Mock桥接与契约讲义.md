...
**专题**：mock.js 与 ChatOrchestrator 对齐  
**需求**：ZL-NA-REQ-022
...
Day 21 后端在 Python，Day 22 尚无 FastAPI。浏览器不能直接 `import orchestrator`。Mock 桥在客户端**模拟** `handle_message` 输出，使 UI 开发不阻塞后端。

```mermaid
sequenceDiagram
    participant B as 浏览器
    participant M as mock.js
    Note over M: Day22 模拟
  participant O as ChatOrchestrator
    Note over O: Day21 真实
    B->>M: sendMessage
    M-->>B: reply 契约字符串
    Note over B,O: Day23 API 连接两者
```

## 2. 契约三要素

1. **前缀语义**：FAQ vs 路由 vs 普通  
2. **可解析性**：`parseReply` 正则稳定  
3. **元数据**：`meta` 显示在气泡底部  

## 3. mock_bridge_demo 用法
...
`sendMessageApi` 已解析此结构。

## 5. 漂移治理

每周同步：后端 FAQ 答案变更 → 更新 Mock 文案 → 产品验收截图。
