...
| 触发正则 | 示例问句 | reply 前缀 | meta |
|----------|----------|------------|------|
| 风险\|有风险 | 投资有风险吗 | [FAQ 直答·72%] | FAQ 直答 |
| 回报率\|年化\|收益 | 年化收益率多少 | [FAQ 直答·74%] | FAQ 直答 |
| 客服\|电话\|联系 | 怎么联系客服 | [FAQ 直答·67%] | FAQ 直答 |

### ROUTE_RULES

| 触发正则 | template | 场景 |
|----------|----------|------|
| 总结\|要点\|摘要 | doc_summary | 文档摘要 |
| 合规\|审阅\|宣传 | compliance_review | 合规审阅 |
| 文档\|资料\|根据 | rag_qa | RAG 问答 |
| 理财\|产品 | product_faq | 产品 FAQ |

## 2. DOM id 与测试映射

| element_id | DOM 角色 | test_frontend 用例 |
|------------|----------|-------------------|
| chat-container | main 包裹 | parametrize |
| messages | 消息列表容器 | parametrize |
| user-input | 文本输入 | parametrize |
| send-btn | 提交按钮 | parametrize |
| chat-form | form 元素 | parametrize |
| loading | 加载指示 | parametrize |

## 3. reply 契约与 parseReply

```mermaid
flowchart TD
    R[reply 字符串] --> P{"startsWith FAQ?"}
    P -->|是| F["kind=faq"]
    P -->|否| Q{"match 路由正则?"}
    Q -->|是| T["kind=route + template"]
    Q -->|否| B["kind=bot"]
```

后端 `handle_message` FAQ 分支：`f"[FAQ 直答·{match.score:.0%}] {match.entry.answer}"`  
后端路由分支：`assistant.chat_turn` 返回 `[路由: {template}] {body}` 格式（由 IntentRouter 决定）。

## 4. Day 23 API 预期 JSON（预习）
...
`mock.js` 的 `sendMessageApi` 已按此字段解析；Day 23 `src/api/chat.py` 须保持一致。

## 5. 验收检查项扩展（讲师用）

1. 打开 DevTools Network，确认仅加载 mock.js、app.js、style.css  
2. 提交空字符串不应发送（HTML5 required + JS trim）  
3. 加载中按钮 disabled，防止重复提交  
4. 消息区自动 scrollToBottom  
5. status-badge 显示 Mock 模式  

## 6. 与 Day 21 FR 对照

| Day 21 | Day 22 前端体现 |
|--------|----------------|
| FR-004 handle_message | mock.js 模拟输出 |
| FAQ 直答阈值 0.65 | Mock 固定百分比展示 |
| auto_route 路由 | ROUTE_RULES 四类 template |
| /tool 调试 | 不在静态页暴露（Day 24+） |
