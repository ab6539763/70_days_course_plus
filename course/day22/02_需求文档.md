...
智链科技 NexusAgent 在 Day 21 交付 `ChatOrchestrator.handle_message` 统一编排入口，但用户界面仍为 CLI。产品要求：在仓库 `frontend/` 搭建**零构建链**静态聊天页，使运营与投资人可在浏览器中体验 FAQ 直答与意图路由，并为 Day 23 FastAPI `POST /api/chat` 预留单一切换点。

## 2. 目标用户

- 终端用户（浏览器输入问题、阅读气泡回复）
- 产品与运营（演示合规 FAQ、路由标签可视化）
- 全栈学员（理解前后端 reply 契约）
- CI（`test_frontend.py` 十二项结构守护）

## 3. 功能需求

### FR-001 页面骨架 index.html

| 项 | 描述 |
|----|------|
| 文件 | `frontend/index.html` |
| 结构 | `header` 品牌区 + `main#chat-container` + `footer.composer` |
| 必选 id | `chat-container`、`messages`、`user-input`、`send-btn`、`chat-form`、`loading` |
| 脚本 | 按序引入 `mock.js`、`app.js` |
| 无障碍 | `aria-live="polite"`、`visually-hidden` label、`role="main"` |
| 欢迎语 | 内置 bot 消息，提示示例问句 |
...
| 布局 | `.app` 纵向 flex，最大宽度 720px 居中 |
| 气泡 | `.msg--user` 右对齐蓝底；`.msg--bot` 左对齐白底边框 |
| 标签 | `.msg__tag--faq` 绿色；`.msg__tag--route` 紫色 |
| 加载 | `.chat__loading` 三点动画，`hidden` 类切换 |
| 响应式 | `@media (max-width: 480px)` 简化副标题 |
...
| 文件 | `frontend/app.js` |
| IIFE | 严格模式，避免全局污染 |
| 函数 | `appendMessage`、`parseReply`、`dispatchMessage`、`handleSubmit` |
| parseReply | 识别 `[FAQ 直答` 前缀、`[路由: xxx]` 正则、默认 bot |
| 加载态 | `setLoading` 禁用输入与按钮，切换 loading 可见性 |
| 键盘 | Enter 提交（非 Shift+Enter） |
| 错误 | catch 后渲染 bot 气泡「错误：…」 |

### FR-004 Mock 桥 mock.js

| 项 | 描述 |
|----|------|
| 文件 | `frontend/mock.js` |
| 导出 | `window.NexusMock = { sendMessage, sendMessageApi, useMock }` |
| FAQ_RULES | 风险、收益、客服三类正则，reply 含 `[FAQ 直答·xx%]` |
| ROUTE_RULES | doc_summary、compliance_review、rag_qa、product_faq |
| 延迟 | `DELAY_MS = 350` 模拟网络 |
| API 占位 | `sendMessageApi` → `fetch('/api/chat', { message })` |
| 契约 | 对齐 `ChatOrchestrator.handle_message` 字符串格式 |
...
| 内容 | `http.server` 启动命令、五文件说明、契约示例、ZL-NA-REQ-022 |

### FR-006 平台审计与演示 day22/*

| 项 | 描述 |
|----|------|
| `constants.py` | `REQUIRED_FILES`、`REQUIRED_ELEMENT_IDS`、`REQ_ID` |
| `frontend_audit.py` | `audit_frontend()` 返回错误列表 |
| `mock_bridge_demo.py` | 并排打印后端 `handle_message` 与 mock 说明 |
| `html_demos.py` | 列出 frontend 文件大小与行数 |

### FR-007 单元测试 test_frontend.py

| 项 | 描述 |
|----|------|
| 文件 | `tests/day22/test_frontend.py` |
| 数量 | 12 条 |
| 覆盖 | 目录存在、五文件、六 id、脚本引入、Mock 函数、FAQ 字符串、appendMessage、CSS 气泡、审计通过、API 占位、README serve 命令 |

## 4. 非功能需求

| 编号 | 要求 |
|------|------|
| NFR-001 | 零 npm/webpack，Python 内置 HTTP 即可预览 |
| NFR-002 | 中文 UI，品牌色 `#1a56db` |
| NFR-003 | Mock 与 API 切换仅改 `useMock` |
| NFR-004 | 单文件 < 15KB，便于 Code Review |
| NFR-005 | 与 Day 21 `handle_message` 前缀契约一致 |
...
全部通过；浏览器手动验证 FAQ 与路由标签。

## 6. 范围外

- Day 23：FastAPI `src/api/chat.py`、CORS、会话  
- WebSocket 流式（Day 16 能力 API 层再接）  
- React/Vue 工程化  
- 用户登录与多会话持久化  
...
```mermaid
graph LR
    D21[Day21 ChatOrchestrator] --> Mock[mock.js 契约]
    Mock --> App[app.js 渲染]
    HTML[index.html] --> App
    CSS[style.css] --> HTML
    App --> UI[浏览器]
    Audit[frontend_audit.py] --> Test[test_frontend]
    D21 --> Bridge[mock_bridge_demo]
```

## 8. 用户故事

**US-022-01** 作为运营，我希望在浏览器输入「投资有风险吗」看到 FAQ 标签，以便向合规演示直答能力。  
**US-022-02** 作为学员，我希望 pytest 守护元素 id，以便 refactor CSS 时不破坏结构。  
**US-022-03** 作为架构师，我希望 `sendMessageApi` 已占位，以便 Day 23 只接后端。

## 9. 风险与缓解

| 风险 | 缓解 |
|------|------|
| Mock 与后端语义漂移 | mock_bridge_demo 对照 |
| 学员改 id 导致测试失败 | 10_ 验收清单强调 |
| 无障碍遗漏 | FR-001 aria 要求 + 19_ 补充 |

## 10. 修订记录

| 版本 | 日期 | 作者 | 说明 |
|------|------|------|------|
| 1.0 | 2026-07-27 | 陈默 | FR-001 至 FR-007 首发 |
