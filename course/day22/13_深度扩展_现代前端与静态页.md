...
```mermaid
graph LR
    Static[纯静态 HTML] --> SPA["React/Vue SPA"]
    SPA --> SSR["Next/Nuxt SSR"]
    Static --> Hybrid[静态+API]
```

Day 22 处于 **静态+API（Hybrid）** 起点：UI 静态，数据来自 API/Mock。
...
Sprint 3 时间盒决定先静态后框架。

## 3. 原生 JS vs 框架

今日模式：**命令式 DOM** — `createElement`、`appendChild`。框架用**声明式** — 状态变则 UI 变。`appendMessage` 即迷你「渲染函数」。

## 4. CSS 方法论

BEM 避免样式冲突。更大项目可用 CSS Modules、Tailwind。智链 Day 22 规模 BEM 足够。

## 5. 可访问性深入

- WCAG 2.1 AA 对比度 4.5:1  
- 键盘：Tab 到 input，Enter 发送  
- 读屏：`aria-live` 于动态区  

## 6. 包管理与未来

Day 30+ 可能引入 Vite + Vue 3。届时 `frontend/` 可迁移为 `web/`，契约 `reply` 不变。

## 7. 推荐阅读

- MDN Learn Web Development  
- Google Web Fundamentals 性能  

扩展完。回归 [11_静态聊天页详解.md](11_静态聊天页详解.md)。
