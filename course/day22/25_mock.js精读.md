...
说明对齐 `ChatOrchestrator.handle_message`，Day 23 替换 fetch。
...
避免污染全局，仅暴露 NexusMock。

## 3. FAQ_RULES 精读

每条：`test` 正则函数、`reply` 含百分比、`meta` 显示用。

**设计选择**：百分比为展示用，与真实 matcher score 无关。

## 4. ROUTE_RULES 精读

`template` 对应 IntentRouter 模板名：`doc_summary`、`compliance_review`、`rag_qa`、`product_faq`。

reply 拼接：`[路由: ${template}] ${body}`

## 5. sendMessage 控制流

```
trim → empty check → delay → FAQ loop → ROUTE loop → default mock
```

## 6. sendMessageApi

fetch POST JSON `{message: text}`，解析 `data.reply/meta/kind`，错误抛 `API 错误: status`。

## 7. 扩展作业

在 ROUTE_RULES 末尾追加规则，注意 FAQ 优先顺序。
...
精读完。配合 [22_Mock桥接与契约讲义.md](22_Mock桥接与契约讲义.md)。
