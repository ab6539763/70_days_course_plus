...
**范围**：`frontend/*` + `src/day22/*` + `tests/day22/test_frontend.py`  
**需求**：ZL-NA-REQ-022

---

## 1. frontend/index.html（62 行）

**走查要点**：
- L8：`link style.css`  
- L23：`id="chat-container"`  
- L24：`id="messages"` + `aria-live`  
- L34：`id="loading"` + `hidden` 初始  
- L43：`id="chat-form"`  
- L46：`id="user-input"` maxlength 2000  
- L53：`id="send-btn"`  
- L59-60：mock.js → app.js  

---

## 2. frontend/style.css（336 行）

**走查要点**：
- L3-18：`:root` 变量  
- L46-54：`.app` flex 列布局  
- L138-145：`.msg--user` / `.msg--bot`  
- L215-223：FAQ/路由标签色  
- L327-335：移动端断点  

---

## 3. frontend/app.js（128 行）

| 行号 | 符号 | 说明 |
|------|------|------|
| 10-14 | DOM 引用 | getElementById |
| 16-18 | scrollToBottom | 滚动 |
| 20-25 | setLoading | 状态 |
| 27-40 | parseReply | 契约 |
| 42-83 | appendMessage | 渲染 |
| 85-94 | dispatchMessage | Mock/API |
| 96-115 | handleSubmit | 主路径 |
| 117-126 | 事件绑定 | submit + keydown |

---

## 4. frontend/mock.js（123 行）
...
| L34-55 | ROUTE_RULES |
| L66-95 | sendMessage |
| L100-115 | sendMessageApi |
| L117-121 | NexusMock 导出 |

---

## 5. src/day22/frontend_audit.py

`audit_frontend()` 遍历 REQUIRED_FILES、检查 id、mock FAQ、appendMessage、CSS 气泡。

---

## 6. src/day22/mock_bridge_demo.py

构建真实 `ChatOrchestrator`，对 QUERIES 打印 `handle_message` 结果前缀。

---

## 7. tests/day22/test_frontend.py
...
讲师提问：「删除 mock.js 中 FAQ 直答字符串，哪条测试红？」  
答案：`test_mock_faq_pattern` 与 `test_frontend_audit_passes`。
