...
sequenceDiagram
    participant U as 用户
    participant F as chat-form
    participant A as app.js
    participant M as mock.js
    participant D as DOM #messages

    U->>F: 输入并提交
    F->>A: submit 事件
    A->> "D: appendMessage(user)"
    A->> "A: setLoading(true)"
    A->> "M: sendMessage(text)"
    M->>M: delay 350ms
    M->>M: FAQ_RULES 匹配
    alt FAQ 命中
        M-->> "A: {reply, meta, kind:faq}"
    else 路由命中
        M-->> "A: {reply, meta, kind:route}"
    else 默认
        M-->> "A: {reply, meta, kind:mock}"
    end
    A->> "A: parseReply(reply)"
    A->> "D: appendMessage(bot, parsed)"
...
```mermaid
flowchart TD
    Start[reply 字符串] --> A{"以 FAQ 直答 开头?"}
    A -->|是| F["kind=faq<br/>渲染绿色标签"]
    A -->|否| B{正则 路由: template}
    B -->|匹配| R["kind=route<br/>渲染紫色标签"]
    B -->|否| G["kind=bot<br/>纯文本气泡"]
```

## 3. Day 22 vs Day 23 数据流对比
...
flowchart LR
    ...
    subgraph d23[Day 23]
        U2[用户] --> P2[静态页]
        P2 --> API["POST /api/chat"]
        API --> O2[ChatOrchestrator]
        O2 --> API
        API --> P2
    end
```
...
```mermaid
flowchart TD
    PR[Pull Request] --> PY[pytest test_frontend]
    PY --> T1["五文件存在?"]
    PY --> T2["六 id 存在?"]
    PY --> T3["mock/app 关键字?"]
    PY --> T4["audit_frontend 空?"]
    T1 & T2 & T3 & T4 --> OK[12 passed]
```

## 6. 四人组协作流

林晓改 HTML/CSS → 陈默 Review 契约 → 赵岩验收文案 → 周航跑 pytest → 合并。

示意图完。实操见 [26_实操Lab手册.md](26_实操Lab手册.md)。
