...
**时长**：90 分钟  
**环境**：浏览器 + `frontend/` + `nexus-agent-platform`
...
浏览器打开 http://localhost:8080 ，确认欢迎语。
...
对比后端输出与浏览器，写 200 字差异分析。

---

## Lab 5：CSS 实验（15 分钟）

修改 `--faq-accent`，观察标签颜色。恢复后提交 diff 到作业 E。

---

## Lab 6：Console 调试（10 分钟）

```javascript
NexusMock.useMock = true;
NexusMock.sendMessage("客服电话").then(console.log);
```
