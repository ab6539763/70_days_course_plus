...
| 顺序 | 文件 | 关注 |
|------|------|------|
| 1 | `expansion_config.py` | validate / defaults |
| 2 | `query_expander.py` | expand + merge |
| 3 | `context.py` | 三阶段 search |
| 4 | `knowledge_store.py` | _build_rag_service 外包 |
| 5 | `api/knowledge.py` | expansion-config |
| 6 | `day35/expansion_demo.py` | 开关对比 |
| 7 | `day35/expansion_api_demo.py` | TestClient |
| 8 | `tests/day35/` | 20 项 |
...
@router.get("/expansion-config", response_model=ExpansionConfigResponse)
def get_route_config() -> ExpansionConfigResponse:
    """返回检索管线路由开关与默认意图"""
...
@router.put("/expansion-config", response_model=ExpansionConfigResponse)
def update_route_config(body: ExpansionConfigRequest) -> ExpansionConfigResponse:
    """更新检索管线路由策略并持久化"""
    store = get_knowledge_store()
    try:
        cfg = ExpansionConfig.from_dict(body.model_dump())
...
@router.post("/expansion-preview", response_model=RoutePreviewResponse)
...
**必读**：`test_expand_produces_multiple_queries_candidates`、`test_expansion_preview_returns_queries`。
...
| 15–40 | query_expander + expanding_retriever |
| 40–55 | context |
...
**Q enabled=False 还构造 ExpandingRetriever 吗？** 构造但不调用 expand。  

---

## 12. 走查验收 oral exam

学员随机抽：讲解 `test_expand_produces_multiple_queries_candidates` 如何构造噪声候选。
