...
1. 改写器类名？ → `RuleBasedAnswerValidator.route`  
2. 管线类名？ → `RAGContextService`  
3. 默认 max_citations？ → `20`  
4. 默认 enabled？ → `True`  
5. 需求号？ → ZL-NA-REQ-037  
6. 平台版本？ → v0.37.0  
7. validation-config HTTP 方法？ → GET + PUT  
8. 打分函数？ → `validate`  
...
**19** 指出 validation-config 两个路由 HTTP 方法。  
**20** 说明 `test_knowledge_store_persists_validation_config` 验证什么。
