...
| 顺序 | 文件 | 关注 |
|------|------|------|
| 1 | `validation_config.py` | validate / defaults |
| 2 | `answer_validator.py` | validate + score |
| 3 | `context.py` | 三阶段 search |
| 4 | `knowledge_store.py` | _build_rag_service 外包 |
| 5 | `api/knowledge.py` | validation-config |
| 6 | `day37/validation_demo.py` | 开关对比 |
| 7 | `day37/validation_api_demo.py` | TestClient |
| 8 | `tests/day37/` | 20 项 |
...
`ValidationConfig` 是改写单一真相源。store 启动 `from_dict` 加载；API PUT 更新。
  ...
  → RAGContextService.retrieve
    → DocumentIndex.search
      → RAGContextService.search
        ...
        → RuleBasedAnswerValidator.route.rewrite
```
...
**必读**：`test_rule_based_validator_passes_good_answer_candidates`、`test_citation_preview_with_rewrite`。
...
| 15–40 | answer_validator + validation_config |
| 40–55 | context |
...
**Q enabled=False 还构造 AnswerValidator 吗？** 构造但不调用 rewrite。  

---

## 12. 走查验收 oral exam

学员随机抽：讲解 `test_rule_based_validator_passes_good_answer_candidates` 如何构造噪声候选。
