...
每一步决策都落进 `executor_trace`，字段设计遵循「thought（为什么）+ 动作（做什么）+ observation（结果）」三元组，方便审计与教学复盘。
...
固定输入集合，对比 `executor_mode` 开/关两种模式下的响应差异，记录延迟与结果准确率。

## 9. 案例：企业级 Agent 平台的分层

```
Stage0: 工具注册（本课 ToolRegistry/StructuredTool）
Stage1: 决策/路由（本课 AgentExecutor）
Stage2: 可观测 trace（本课 executor_trace）
