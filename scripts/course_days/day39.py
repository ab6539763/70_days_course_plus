#!/usr/bin/env python3
"""Gold-standard course material builder for Day 39 — 手写 ReAct Agent 可观测工具链。"""
...
REACT_AGENT = read_repo(f"{REPO}/agent/react_agent.py")
REACT_CONFIG = read_repo(f"{REPO}/agent/react_config.py")
...
REACT_DEMO = read_repo(f"{REPO}/day39/react_demo.py")
REACT_API_DEMO = read_repo(f"{REPO}/day39/react_api_demo.py")
TEST_REACT = read_repo(f"{REPO}/../tests/day39/test_react_agent.py")
TEST_REACT_API = read_repo(f"{REPO}/../tests/day39/test_react_api.py")
VALIDATION_DEMO = REACT_DEMO
VALIDATION_API_DEMO = REACT_API_DEMO
TEST_VALIDATOR = TEST_REACT
TEST_VALIDATION_API = TEST_REACT_API
ANSWER_VALIDATOR = REACT_AGENT
VALIDATION_RETRY = REACT_AGENT

_AGENT_API = AGENT_API[
    AGENT_API.find('@router.get("/react-config"'): AGENT_API.find(
        '@router.post("/react-preview"'
    )
    + len('@router.post("/react-preview"')
]
_VALIDATOR_CLASS = REACT_AGENT[
    REACT_AGENT.find("class ReactStep"): REACT_AGENT.find("class ReActAgent")
]
_VALIDATION_CFG_METHODS = KNOWLEDGE_STORE[
    KNOWLEDGE_STORE.find("def get_react_config"): KNOWLEDGE_STORE.find(
        "def validate_answer"
    )
]
_CHAT_VALIDATION = CHAT_API[
    CHAT_API.find("if body.agent_mode"): CHAT_API.find(
        "cite_data = store.fetch_citations"
    )
]
_FETCH_CITATIONS_RETRY = REACT_AGENT[
    REACT_AGENT.find("def run("): REACT_AGENT.find("def _plan_step")
]
...
「打开 react_agent，看主循环。先看 enabled 分支；开则进入 Thought→Action→Observation 三段。Action 调 ToolExecutor，Observation 写回 trace，截断 max_steps。这就是 {REQ} 的执行路径。」
...
**NexusAgent 课程 · Phase 4 · Day 39 · ReAct · {REQ} · react_agent 精读完**
"""
...
def _file27() -> str:
    return f"""# Day 40 预习：LangChain Agent 工具注册
...
if __name__ == "__main__":
    write_course(39, build(), min_chars=100_000)
