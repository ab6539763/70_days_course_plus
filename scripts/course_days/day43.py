#!/usr/bin/env python3
"""Gold-standard course material builder for Day 43 — Supervisor 多 Agent 委派。"""
...
SUPERVISOR_GRAPH = read_repo(f"{REPO}/agent/supervisor_graph.py")
SUPERVISOR_CONFIG = read_repo(f"{REPO}/agent/supervisor_config.py")
SUB_AGENT = read_repo(f"{REPO}/agent/sub_agent.py")
SUPERVISOR_STATE = read_repo(f"{REPO}/agent/supervisor_state.py")
AGENT_API = read_repo(f"{REPO}/api/agent.py")
VALIDATION_RETRY = read_repo(f"{REPO}/rag/validation_retry.py")
...
REACT_DEMO = read_repo(f"{REPO}/day43/supervisor_demo.py")
REACT_API_DEMO = read_repo(f"{REPO}/day43/supervisor_api_demo.py")
TEST_REACT = read_repo(f"{REPO}/../tests/day43/test_supervisor_graph.py")
TEST_REACT_API = read_repo(f"{REPO}/../tests/day43/test_supervisor_api.py")
APPROVAL_GRAPH = SUPERVISOR_GRAPH
APPROVAL_CONFIG = SUPERVISOR_CONFIG
APPROVAL_CHECKPOINT = SUB_AGENT
STATE_GRAPH = SUPERVISOR_GRAPH
GRAPH_STATE = SUPERVISOR_STATE
GRAPH_AGENT = SUPERVISOR_GRAPH
GRAPH_CONFIG = SUPERVISOR_CONFIG
REACT_AGENT = SUPERVISOR_GRAPH
REACT_CONFIG = SUPERVISOR_CONFIG
EXECUTOR_AGENT = SUPERVISOR_GRAPH
STRUCTURED_TOOL = SUB_AGENT
EXECUTOR_CONFIG = SUPERVISOR_CONFIG
TOOL_ADAPTER = SUPERVISOR_STATE
VALIDATION_DEMO = REACT_DEMO
VALIDATION_API_DEMO = REACT_API_DEMO
TEST_VALIDATOR = TEST_REACT
TEST_VALIDATION_API = TEST_REACT_API
ANSWER_VALIDATOR = SUPERVISOR_GRAPH
VALIDATION_RETRY = SUPERVISOR_GRAPH

_AGENT_API = AGENT_API[
    AGENT_API.find('@router.get("/supervisor-config"'): AGENT_API.find(
        '@router.post("/supervisor-preview"'
    )
    + len('@router.post("/supervisor-preview"')
]
_VALIDATOR_CLASS = SUPERVISOR_GRAPH[
    SUPERVISOR_GRAPH.find("class SupervisorGraph"): SUPERVISOR_GRAPH.find("def _build_graph")
]
_VALIDATION_CFG_METHODS = KNOWLEDGE_STORE[
    KNOWLEDGE_STORE.find("def get_supervisor_config"): KNOWLEDGE_STORE.find(
        "def validate_answer"
    )
]
_CHAT_VALIDATION = CHAT_API[
    CHAT_API.find("if body.supervisor_mode"): CHAT_API.find(
        "cite_data = store.fetch_citations"
    )
]
_FETCH_CITATIONS_RETRY = SUPERVISOR_GRAPH[
    SUPERVISOR_GRAPH.find("def invoke("): SUPERVISOR_GRAPH.find("def _empty_outcome")
]

# 兼容课件模板中的变量名（由 Day38 模板迁移）
ROUTE_CONFIG = REACT_CONFIG
QUERY_ROUTER = REACT_AGENT
ROUTING_RETRIEVER = REACT_AGENT
ROUTE_DEMO = REACT_DEMO
ROUTE_API_DEMO = REACT_API_DEMO
TEST_ROUTER = TEST_REACT
TEST_ROUTE_API = TEST_REACT_API
_ROUTE_API = _AGENT_API
_ROUTE_CLASS = _VALIDATOR_CLASS
_ROUTE_CFG_METHODS = _VALIDATION_CFG_METHODS
_ROUTING_SEARCH = _FETCH_CITATIONS_RETRY
_BUILD_RAG = _VALIDATION_CFG_METHODS
_CHAT_CITATIONS = _CHAT_VALIDATION
_FETCH_CITATIONS = _FETCH_CITATIONS_RETRY
...
def build() -> dict[str, str]:
    files = {
        ...
        "08_作业.md": _file08(),
        "09_作业答案.md": _file09(),
        "10_Supervisor验收清单.md": _file10(),
        "11_Supervisor详解.md": _file11(),
        "12_课堂练习册.md": _file12(),
        "13_深度扩展_Supervisor方法论.md": _file13(),
        "14_企业案例集_多Agent协作场景.md": _file14(),
        "15_授课实录.md": _file15(),
        "16_复习卡片.md": _file16(),
        "17_Supervisor_API速查手册.md": _file17(),
        "18_与Day42能力对照表.md": _file18(),
        "19_讲师补充阅读.md": _file19(),
        "20_完整代码走查.md": _file20(),
        "21_课堂知识竞赛.md": _file21(),
        "22_supervisor_graph精读.md": _file22(),
        "23_步数上限与延迟预算实践.md": _file23(),
        "24_Phase4第五日总结.md": _file24(),
        "25_supervisor_graph_api脚本精读.md": _file25(),
        "26_实操Lab手册.md": _file26(),
        "27_Day44预习.md": _file27(),
    }
    return files
...
「打开 supervisor_graph，看委派路由。先看 supervisor_route；按关键词委派给 faq_worker/rag_worker/intent_worker 之一，synthesize 汇总。这就是 {REQ} 的执行路径。」
...
**NexusAgent 课程 · Phase 4 · Day 43 · Supervisor · {REQ} · supervisor_graph 精读完**
"""
...
def _file27() -> str:
    return f"""# Day 44 预习：MCP 协议与工具生态
