#!/usr/bin/env python3
"""Gold-standard course material builder for Day 44 — MCP 协议与工具生态。"""
...
MCP_CONFIG = read_repo(f"{REPO}/agent/mcp_config.py")
MCP_SERVER = read_repo(f"{REPO}/agent/mcp_server.py")
MCP_PROTOCOL = read_repo(f"{REPO}/agent/mcp_protocol.py")
MCP_BRIDGE = read_repo(f"{REPO}/agent/mcp_bridge.py")
...
REACT_DEMO = read_repo(f"{REPO}/day44/mcp_demo.py")
REACT_API_DEMO = read_repo(f"{REPO}/day44/mcp_api_demo.py")
TEST_REACT = read_repo(f"{REPO}/../tests/day44/test_mcp_server.py")
TEST_REACT_API = read_repo(f"{REPO}/../tests/day44/test_mcp_api.py")
APPROVAL_GRAPH = MCP_RUNNER
APPROVAL_CONFIG = MCP_CONFIG
APPROVAL_CHECKPOINT = MCP_SERVER
STATE_GRAPH = MCP_RUNNER
GRAPH_STATE = MCP_PROTOCOL
GRAPH_AGENT = MCP_RUNNER
GRAPH_CONFIG = MCP_CONFIG
REACT_AGENT = MCP_RUNNER
REACT_CONFIG = MCP_CONFIG
EXECUTOR_AGENT = MCP_RUNNER
STRUCTURED_TOOL = MCP_SERVER
EXECUTOR_CONFIG = MCP_CONFIG
TOOL_ADAPTER = MCP_PROTOCOL
VALIDATION_DEMO = REACT_DEMO
VALIDATION_API_DEMO = REACT_API_DEMO
TEST_VALIDATOR = TEST_REACT
TEST_VALIDATION_API = TEST_REACT_API
ANSWER_VALIDATOR = MCP_RUNNER
VALIDATION_RETRY = MCP_RUNNER

_AGENT_API = AGENT_API[
    AGENT_API.find('@router.get("/mcp-config"'): AGENT_API.find(
        '@router.post("/mcp-preview"'
    )
    + len('@router.post("/mcp-preview"')
]
_VALIDATOR_CLASS = MCP_RUNNER[
    MCP_RUNNER.find("class McpRunner"): MCP_RUNNER.find("def _empty_outcome")
]
_VALIDATION_CFG_METHODS = KNOWLEDGE_STORE[
    KNOWLEDGE_STORE.find("def get_mcp_config"): KNOWLEDGE_STORE.find(
        "def validate_answer"
    )
]
_CHAT_VALIDATION = CHAT_API[
    CHAT_API.find("if body.mcp_mode"): CHAT_API.find(
        "cite_data = store.fetch_citations"
    )
]
_FETCH_CITATIONS_RETRY = MCP_RUNNER[
    MCP_RUNNER.find("def invoke("): MCP_RUNNER.find("def _empty_outcome")
]
...
def _file27() -> str:
    return f"""# Day 45 预习：Dify 工作流对接与周测
...
if __name__ == "__main__":
    write_course(44, build(), min_chars=100_000)
