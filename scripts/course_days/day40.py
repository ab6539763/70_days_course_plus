#!/usr/bin/env python3
"""Gold-standard course material builder for Day 40 — AgentExecutor 框架式工具注册。"""
...
EXECUTOR_AGENT = read_repo(f"{REPO}/agent/agent_executor.py")
STRUCTURED_TOOL = read_repo(f"{REPO}/agent/structured_tool.py")
EXECUTOR_CONFIG = read_repo(f"{REPO}/agent/executor_config.py")
TOOL_ADAPTER = read_repo(f"{REPO}/agent/tool_adapter.py")
...
REACT_DEMO = read_repo(f"{REPO}/day40/executor_demo.py")
REACT_API_DEMO = read_repo(f"{REPO}/day40/executor_api_demo.py")
TEST_REACT = read_repo(f"{REPO}/../tests/day40/test_agent_executor.py")
TEST_REACT_API = read_repo(f"{REPO}/../tests/day40/test_executor_api.py")
REACT_AGENT = EXECUTOR_AGENT
REACT_CONFIG = EXECUTOR_CONFIG
VALIDATION_DEMO = REACT_DEMO
VALIDATION_API_DEMO = REACT_API_DEMO
TEST_VALIDATOR = TEST_REACT
TEST_VALIDATION_API = TEST_REACT_API
ANSWER_VALIDATOR = EXECUTOR_AGENT
VALIDATION_RETRY = EXECUTOR_AGENT

_AGENT_API = AGENT_API[
    AGENT_API.find('@router.get("/executor-config"'): AGENT_API.find(
        '@router.post("/executor-preview"'
    )
    + len('@router.post("/executor-preview"')
]
_VALIDATOR_CLASS = EXECUTOR_AGENT[
    EXECUTOR_AGENT.find("class ExecutorStep"): EXECUTOR_AGENT.find("class AgentExecutor")
]
_VALIDATION_CFG_METHODS = KNOWLEDGE_STORE[
    KNOWLEDGE_STORE.find("def get_executor_config"): KNOWLEDGE_STORE.find(
        "def validate_answer"
    )
]
_CHAT_VALIDATION = CHAT_API[
    CHAT_API.find("if body.executor_mode"): CHAT_API.find(
        "cite_data = store.fetch_citations"
    )
]
_FETCH_CITATIONS_RETRY = EXECUTOR_AGENT[
    EXECUTOR_AGENT.find("def invoke("): EXECUTOR_AGENT.find("def _plan_step")
]
...
def build() -> dict[str, str]:
    files = {
        ...
        "08_作业.md": _file08(),
        "09_作业答案.md": _file09(),
        "10_Executor验收清单.md": _file10(),
        "11_Executor详解.md": _file11(),
        "12_课堂练习册.md": _file12(),
        "13_深度扩展_Executor方法论.md": _file13(),
        "14_企业案例集_StructuredTool调用场景.md": _file14(),
        "15_授课实录.md": _file15(),
        "16_复习卡片.md": _file16(),
        "17_Executor_API速查手册.md": _file17(),
        "18_与Day39能力对照表.md": _file18(),
        "19_讲师补充阅读.md": _file19(),
        "20_完整代码走查.md": _file20(),
        "21_课堂知识竞赛.md": _file21(),
        "22_agent_executor精读.md": _file22(),
        "23_步数上限与延迟预算实践.md": _file23(),
        "24_Phase4第二日总结.md": _file24(),
        "25_agent_executor_api脚本精读.md": _file25(),
        "26_实操Lab手册.md": _file26(),
        "27_Day41预习.md": _file27(),
    }
    return files
...
「打开 agent_executor，看主循环。先看 enabled 分支；开则用 StructuredTool 调工具，intermediate_steps 记录每一步，截断 max_iterations。这就是 {REQ} 的执行路径。」
...
**NexusAgent 课程 · Phase 4 · Day 40 · AgentExecutor · {REQ} · agent_executor 精读完**
"""
...
def _file27() -> str:
    return f"""# Day 41 预习：LangGraph 状态图编排
...
if __name__ == "__main__":
    write_course(40, build(), min_chars=100_000)
