#!/usr/bin/env python3
"""Gold-standard course material builder for Day 38 — 多轮 Self-RAG 校验重试 Validation Retry."""
...
RETRY_DEMO = read_repo(f"{REPO}/day38/retry_demo.py")
RETRY_API_DEMO = read_repo(f"{REPO}/day38/retry_api_demo.py")
TEST_RETRY = read_repo(f"{REPO}/../tests/day38/test_validation_retry.py")
TEST_RETRY_API = read_repo(f"{REPO}/../tests/day38/test_retry_api.py")
VALIDATION_DEMO = RETRY_DEMO
VALIDATION_API_DEMO = RETRY_API_DEMO
TEST_VALIDATOR = TEST_RETRY
TEST_VALIDATION_API = TEST_RETRY_API
...
_CHAT_VALIDATION = CHAT_API[
    CHAT_API.find("outcome = apply_validation_retry"): CHAT_API.find(
        "return ChatResponse"
    )
]
_FETCH_CITATIONS_RETRY = KNOWLEDGE_STORE[
    KNOWLEDGE_STORE.find("def fetch_citations_retry"): KNOWLEDGE_STORE.find(
        "def ingest_text"
    )
]
...
QUERY_ROUTER = VALIDATION_RETRY
ROUTING_RETRIEVER = VALIDATION_RETRY
...
_ROUTING_SEARCH = VALIDATION_RETRY[
    VALIDATION_RETRY.find("def apply_validation_retry"): VALIDATION_RETRY.rfind(
        "return ValidationRetryOutcome"
    )
    + len("return ValidationRetryOutcome")
]
_BUILD_RAG = _FETCH_CITATIONS_RETRY
...
def build() -> dict[str, str]:
    files = {
        ...
        "08_作业.md": _file08(),
        "09_作业答案.md": _file09(),
        "10_Retry验收清单.md": _file10(),
        "11_校验重试详解.md": _file11(),
        "12_课堂练习册.md": _file12(),
        "13_深度扩展_多轮Self-RAG方法论.md": _file13(),
        "14_企业案例集_校验失败恢复场景.md": _file14(),
        "15_授课实录.md": _file15(),
        "16_复习卡片.md": _file16(),
        "17_Retry_API速查手册.md": _file17(),
        "18_与Day37能力对照表.md": _file18(),
        "19_讲师补充阅读.md": _file19(),
        "20_完整代码走查.md": _file20(),
        "21_课堂知识竞赛.md": _file21(),
        "22_validation_retry精读.md": _file22(),
        "23_重试阈值与拒答实践.md": _file23(),
        "24_Phase3第十三日总结.md": _file24(),
        "25_retry_api脚本精读.md": _file25(),
        "26_实操Lab手册.md": _file26(),
        "27_Day39预习.md": _file27(),
    }
    return files
...
「打开 validation_retry，找 apply_validation_retry。先看首次校验：通过直接返回。失败且 retry_on_fail 才用 rag_wide 重检索再校验一次。这就是 {REQ} 的读取路径。」
...
「打开 validation_retry，apply_validation_retry 返回是否重试过。chat 里 validation.retries 挂在 reply 后面。前端可展示重试次数。这就是 {REQ}。」

---

## 五十、End of 22 精读

**NexusAgent 课程 · Phase 3 · Day 38 · Retry · {REQ} · validation_retry 精读完**
"""
...
def _file27() -> str:
    return f"""# Day 39 预习：Agent 工具链深化
...
if __name__ == "__main__":
    write_course(38, build(), min_chars=100_000)
