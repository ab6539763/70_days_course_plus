#!/usr/bin/env python3
"""Gold-standard course material builder for Day 42 — 人工审批工作流。"""
...
APPROVAL_GRAPH = read_repo(f"{REPO}/agent/approval_workflow_graph.py")
APPROVAL_CONFIG = read_repo(f"{REPO}/agent/approval_config.py")
APPROVAL_CHECKPOINT = read_repo(f"{REPO}/agent/approval_checkpoint.py")
STATE_GRAPH = read_repo(f"{REPO}/agent/state_graph.py")
GRAPH_STATE = read_repo(f"{REPO}/agent/graph_state.py")
...
REACT_DEMO = read_repo(f"{REPO}/day42/approval_demo.py")
REACT_API_DEMO = read_repo(f"{REPO}/day42/approval_api_demo.py")
TEST_REACT = read_repo(f"{REPO}/../tests/day42/test_approval_workflow.py")
TEST_REACT_API = read_repo(f"{REPO}/../tests/day42/test_approval_api.py")
GRAPH_AGENT = APPROVAL_GRAPH
GRAPH_CONFIG = APPROVAL_CONFIG
REACT_AGENT = APPROVAL_GRAPH
REACT_CONFIG = APPROVAL_CONFIG
EXECUTOR_AGENT = APPROVAL_GRAPH
STRUCTURED_TOOL = STATE_GRAPH
EXECUTOR_CONFIG = APPROVAL_CONFIG
TOOL_ADAPTER = GRAPH_STATE
VALIDATION_DEMO = REACT_DEMO
VALIDATION_API_DEMO = REACT_API_DEMO
TEST_VALIDATOR = TEST_REACT
TEST_VALIDATION_API = TEST_REACT_API
ANSWER_VALIDATOR = APPROVAL_GRAPH
VALIDATION_RETRY = APPROVAL_GRAPH

_AGENT_API = AGENT_API[
    AGENT_API.find('@router.get("/approval-config"'): AGENT_API.find(
        '@router.post("/approval-resume"'
    )
    + len('@router.post("/approval-resume"')
]
_VALIDATOR_CLASS = APPROVAL_GRAPH[
    APPROVAL_GRAPH.find("class ApprovalWorkflowGraph"): APPROVAL_GRAPH.find("def _build_graph")
]
_VALIDATION_CFG_METHODS = KNOWLEDGE_STORE[
    KNOWLEDGE_STORE.find("def get_approval_config"): KNOWLEDGE_STORE.find(
        "def validate_answer"
    )
]
_CHAT_VALIDATION = CHAT_API[
    CHAT_API.find("if body.approval_mode"): CHAT_API.find(
        "cite_data = store.fetch_citations"
    )
]
_FETCH_CITATIONS_RETRY = APPROVAL_GRAPH[
    APPROVAL_GRAPH.find("def invoke("): APPROVAL_GRAPH.find("def resume")
]
...
def build() -> dict[str, str]:
    files = {
        ...
        "08_作业.md": _file08(),
        "09_作业答案.md": _file09(),
        "10_Approval验收清单.md": _file10(),
        "11_Approval详解.md": _file11(),
        "12_课堂练习册.md": _file12(),
        "13_深度扩展_Approval方法论.md": _file13(),
        "14_企业案例集_审批卡点场景.md": _file14(),
        "15_授课实录.md": _file15(),
        "16_复习卡片.md": _file16(),
        "17_Approval_API速查手册.md": _file17(),
        "18_与Day41能力对照表.md": _file18(),
        "19_讲师补充阅读.md": _file19(),
        "20_完整代码走查.md": _file20(),
        "21_课堂知识竞赛.md": _file21(),
        "22_approval_workflow精读.md": _file22(),
        "23_步数上限与延迟预算实践.md": _file23(),
        "24_Phase4第四日总结.md": _file24(),
        "25_approval_workflow_api脚本精读.md": _file25(),
        "26_实操Lab手册.md": _file26(),
        "27_Day43预习.md": _file27(),
    }
    return files
...
「打开 approval_workflow_graph，看审批节点。先看 require_rag_approval；命中则中断并返回 checkpoint_id，resume 后续跑。这就是 {REQ} 的执行路径。」
...
**NexusAgent 课程 · Phase 4 · Day 42 · Approval · {REQ} · approval_workflow 精读完**
"""
...
def _file27() -> str:
    return f"""# Day 43 预习：多 Agent Supervisor 模式
...
if __name__ == "__main__":
    write_course(42, build(), min_chars=100_000)
