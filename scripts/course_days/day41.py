#!/usr/bin/env python3
"""Gold-standard course material builder for Day 41 — StateGraph 状态图编排。"""
...
GRAPH_AGENT = read_repo(f"{REPO}/agent/rag_agent_graph.py")
STATE_GRAPH = read_repo(f"{REPO}/agent/state_graph.py")
GRAPH_CONFIG = read_repo(f"{REPO}/agent/graph_config.py")
...
REACT_DEMO = read_repo(f"{REPO}/day41/graph_demo.py")
REACT_API_DEMO = read_repo(f"{REPO}/day41/graph_api_demo.py")
TEST_REACT = read_repo(f"{REPO}/../tests/day41/test_state_graph.py")
TEST_REACT_API = read_repo(f"{REPO}/../tests/day41/test_graph_api.py")
REACT_AGENT = GRAPH_AGENT
REACT_CONFIG = GRAPH_CONFIG
EXECUTOR_AGENT = GRAPH_AGENT
STRUCTURED_TOOL = STATE_GRAPH
EXECUTOR_CONFIG = GRAPH_CONFIG
...
ANSWER_VALIDATOR = GRAPH_AGENT
VALIDATION_RETRY = GRAPH_AGENT

_AGENT_API = AGENT_API[
    AGENT_API.find('@router.get("/graph-config"'): AGENT_API.find(
        '@router.post("/graph-preview"'
    )
    + len('@router.post("/graph-preview"')
]
_VALIDATOR_CLASS = GRAPH_AGENT[
    GRAPH_AGENT.find("class RAGAgentGraph"): GRAPH_AGENT.find("def _build_graph")
]
_VALIDATION_CFG_METHODS = KNOWLEDGE_STORE[
    KNOWLEDGE_STORE.find("def get_graph_config"): KNOWLEDGE_STORE.find(
        "def validate_answer"
    )
]
_CHAT_VALIDATION = CHAT_API[
    CHAT_API.find("if body.graph_mode"): CHAT_API.find(
        "cite_data = store.fetch_citations"
    )
]
_FETCH_CITATIONS_RETRY = GRAPH_AGENT[
    GRAPH_AGENT.find("def invoke("): GRAPH_AGENT.find("def _build_graph")
]
...
def build() -> dict[str, str]:
    files = {
        ...
        "08_作业.md": _file08(),
        "09_作业答案.md": _file09(),
        "10_Graph验收清单.md": _file10(),
        "11_Graph详解.md": _file11(),
        "12_课堂练习册.md": _file12(),
        "13_深度扩展_Graph方法论.md": _file13(),
        "14_企业案例集_多节点Agent场景.md": _file14(),
        "15_授课实录.md": _file15(),
        "16_复习卡片.md": _file16(),
        "17_Graph_API速查手册.md": _file17(),
        "18_与Day40能力对照表.md": _file18(),
        "19_讲师补充阅读.md": _file19(),
        "20_完整代码走查.md": _file20(),
        "21_课堂知识竞赛.md": _file21(),
        "22_rag_agent_graph精读.md": _file22(),
        "23_步数上限与延迟预算实践.md": _file23(),
        "24_Phase4第三日总结.md": _file24(),
        "25_rag_agent_graph_api脚本精读.md": _file25(),
        "26_实操Lab手册.md": _file26(),
        "27_Day42预习.md": _file27(),
    }
    return files
...
「打开 rag_agent_graph，看节点图。先看 entry_point；planner 决策后走条件边到 tool_runner 或 answer，node_path 记录轨迹。这就是 {REQ} 的执行路径。」
...
**NexusAgent 课程 · Phase 4 · Day 41 · StateGraph · {REQ} · rag_agent_graph 精读完**
"""
...
def _file27() -> str:
    return f"""# Day 42 预习：LangGraph 进阶 — 人工审批工作流
