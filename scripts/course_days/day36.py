#!/usr/bin/env python3
"""Gold-standard course material builder for Day 36 — 自适应路由 Query Router."""
...
QUERY_ROUTER = read_repo(f"{REPO}/rag/query_router.py")
ROUTE_CONFIG = read_repo(f"{REPO}/rag/route_config.py")
...
ROUTE_DEMO = read_repo(f"{REPO}/day36/route_demo.py")
ROUTE_API_DEMO = read_repo(f"{REPO}/day36/route_api_demo.py")
TEST_ROUTER = read_repo(f"{REPO}/../tests/day36/test_query_router.py")
TEST_ROUTE_API = read_repo(f"{REPO}/../tests/day36/test_route_api.py")
...
_ROUTE_API = KNOWLEDGE_API[
    KNOWLEDGE_API.find('@router.get("/route-config"'): KNOWLEDGE_API.find(
        '@router.get("/chunk-config"'
    )
]
ROUTING_RETRIEVER = read_repo(f"{REPO}/rag/routing_retriever.py")
ROUTE_CONFIG = read_repo(f"{REPO}/rag/route_config.py")
_ROUTING_SEARCH = ROUTING_RETRIEVER[
    ROUTING_RETRIEVER.find("def search(self"): ROUTING_RETRIEVER.rfind("return RuleBasedQueryRouter.route")
    + len("return RuleBasedQueryRouter.route(batches, top_k=top_k)")
]
_ROUTE_CLASS = QUERY_ROUTER[
    QUERY_ROUTER.find("class ExpansionResult"): QUERY_ROUTER.find("class QueryRouter")
]
_ROUTE_CFG_METHODS = KNOWLEDGE_STORE[
    KNOWLEDGE_STORE.find("def get_route_config"): KNOWLEDGE_STORE.find(
        "def fetch_citations")
]
...
def build() -> dict[str, str]:
    files = {
        ...
        "08_作业.md": _file08(),
        "09_作业答案.md": _file09(),
        "10_Route验收清单.md": _file10(),
        "11_自适应路由详解.md": _file11(),
        "12_课堂练习册.md": _file12(),
        ...
        "22_query_router精读.md": _file22(),
        "23_路由决策与SLA实践.md": _file23(),
        "24_Phase3第十二日总结.md": _file24(),
        "25_route_api脚本精读.md": _file25(),
        "26_实操Lab手册.md": _file26(),
        "27_Day37预习.md": _file27(),
    }
    return files


def _readme() -> str:
    return f"""# Day 36 课件索引
...
智链 NexusAgent {VER} 上线自适应检索路由，简单 FAQ 延迟降低 60%，复杂问句保持宽召回。
...
| AC-06 | 版本 | GET /api/health | 0.36.0 |
...
set -e
export PYTHONPATH=src NEXUS_LLM_MOCK=1
pytest tests/day36/ -q
python3 src/day36/route_demo.py | grep -q "✅"
python3 src/day36/route_api_demo.py | grep -q "✅"
echo DAY36_OK
```
...
「打开 RoutingRetriever，找 search。先看 intent：faq_fast 直接查，rag_standard 走默认管线，rag_wide 才开 expand+rewrite。这就是 {REQ} 的读取路径。」
...
「打开 query_router，RuleBasedQueryRouter.route 返回 intent。chat 里 route 字段挂在 reply 后面。前端可展示当前走的是快路径还是宽路径。这就是 {REQ}。」

---

## 五十、End of 22 精读

**NexusAgent 课程 · Phase 3 · Day 36 · Route · {REQ} · query_router 精读完**
"""
...
{REQ} 已交付：QueryRouter、citation API、chat citations、前端展示、20 测试。下一步：Day35 多 query 扩展。
"""


def _file25() -> str:
    return f"""# Day 36 API 脚本精读
...
```bash
python3 src/day36/route_demo.py | tee /tmp/day37_demo.txt
```
...
def _file27() -> str:
    return f"""# Day 37 预习：Self-RAG 与答案校验
