#!/usr/bin/env python3
"""Gold-standard course material builder for Day 35 — 多查询扩展 HyDE / Query Expansion."""
...
QUERY_EXPANDER = read_repo(f"{REPO}/rag/query_expander.py")
EXPANSION_CONFIG = read_repo(f"{REPO}/rag/expansion_config.py")
...
EXPANSION_DEMO = read_repo(f"{REPO}/day35/expansion_demo.py")
EXPANSION_API_DEMO = read_repo(f"{REPO}/day35/expansion_api_demo.py")
TEST_EXPANDER = read_repo(f"{REPO}/../tests/day35/test_query_expander.py")
TEST_EXPANSION_API = read_repo(f"{REPO}/../tests/day35/test_expansion_api.py")
...
_EXPANSION_API = KNOWLEDGE_API[
    KNOWLEDGE_API.find('@router.get("/expansion-config"'): KNOWLEDGE_API.find(
        '@router.get("/chunk-config"'
    )
]
EXPANDING_RETRIEVER = read_repo(f"{REPO}/rag/expanding_retriever.py")
RESULT_MERGER = read_repo(f"{REPO}/rag/result_merger.py")
_EXPANDING_SEARCH = EXPANDING_RETRIEVER[
    EXPANDING_RETRIEVER.find("def search(self"): EXPANDING_RETRIEVER.rfind("return merge_retrieval_results")
    + len("return merge_retrieval_results(batches, top_k=top_k)")
]
_EXPANSION_CLASS = QUERY_EXPANDER[
    QUERY_EXPANDER.find("class ExpansionResult"): QUERY_EXPANDER.find("class QueryExpander")
]
_EXPANSION_CFG_METHODS = KNOWLEDGE_STORE[
    KNOWLEDGE_STORE.find("def get_expansion_config"): KNOWLEDGE_STORE.find(
        "def fetch_citations")
]
...
def build() -> dict[str, str]:
    files = {
        ...
        "08_作业.md": _file08(),
        "09_作业答案.md": _file09(),
        "10_Expansion验收清单.md": _file10(),
        "11_多查询扩展详解.md": _file11(),
        "12_课堂练习册.md": _file12(),
        "13_深度扩展_HyDE与召回方法论.md": _file13(),
        "14_企业案例集_宽召回场景.md": _file14(),
        "15_授课实录.md": _file15(),
        "16_复习卡片.md": _file16(),
        "17_Expansion_API速查手册.md": _file17(),
        "18_与Day34能力对照表.md": _file18(),
        "19_讲师补充阅读.md": _file19(),
        "20_完整代码走查.md": _file20(),
        "21_课堂知识竞赛.md": _file21(),
        "22_query_expander精读.md": _file22(),
        "23_多路合并与延迟实践.md": _file23(),
        "24_Phase3第十一日总结.md": _file24(),
        "25_expansion_api脚本精读.md": _file25(),
        "26_实操Lab手册.md": _file26(),
        "27_Day36预习.md": _file27(),
    }
    return files


def _readme() -> str:
    return f"""# Day 35 课件索引
...
智链 NexusAgent {VER} 上线多查询扩展，金融 FAQ 宽召回能力提升，citations 合并去重可审计。
...
Day 33 查询改写提升检索质量，但 chat 仅返回 `reply` 字符串，质检无法核对依据来源。合规要求 RAG 回复附带 **可追溯引用** `expansion.queries + merged citations`。教学栈用 `QueryExpander` 将 `RetrievalResult` 格式化为结构化引用，并可选附带 rewrite 审计元数据。

## 2. 目标

- 检索 top-k → `merge_retrieval_results` → `expansion.queries + merged citations`（默认 3 条）  
- `ExpansionConfig` 可开关、调条数、调 preview 长度  
- `expansion-config` / `citation-preview` REST API  
- `POST /api/chat` 返回 `reply` + `citations` + `rewrite`  
- 前端 `msg__expansion` 展示引用列表  

## 3. 功能需求

### FR-001 Citation 数据模型与 merge_retrieval_results

- `Citation` dataclass：rank、chunk_id、source、score、preview、matched_tokens  
- `merge_retrieval_results(results, preview_max_chars, max_items)` 截断 preview  
- `CitationBundle` 含 citations + 可选 rewrite  

### FR-002 ExpandingRetriever.search

- `RAGContextService.ExpandingRetriever.search(query, config)`  
- 调用 `index.search` 获取 top-k  
- 若 retriever 为 `RewritingRetriever`，附加 `last_rewrite`  

### FR-003 ExpansionConfig 与校验

- 字段：`enabled`, `max_citations`, `preview_max_chars`, `include_expansion_meta`  
...
### FR-005 expansion-config / citation-preview REST API
...
- `day36/expansion_demo.py` 打印引用列表  
- `day36/expansion_api_demo.py` 演示 API  
- `tests/day35/` 20 项覆盖 builder、store、API、chat  
...
- status / expansion-config 可读 enabled 与 max_citations  
- expansion_demo stdout 标注 rank、source、preview  
...
- `merge_retrieval_results` 可单测  
- `test_chat_includes_citations` 验证端到端  

### NFR-005 安全

- expansion-config 仅改展示策略  
- preview 长度上限防滥用  

## 5. 非目标

- 自动生成 footnote 编号插入 reply 正文（Phase 4）  
- 跨文档引用合并去重（Day 36 HyDE）  
- PDF 页码级定位  
...
| FR | 实现位置 | 测试 |
|----|----------|------|
| FR-001 | citation_builder.merge_retrieval_results | test_merge_retrieval_results_from_results |
| FR-002 | context.ExpandingRetriever.search | test_ExpandingRetriever.search_rewrite_meta |
...
| FR-007 | day36/* | test_health_version |
...
```bash
pytest tests/day35/test_query_expander.py -v
...
**新增**：QueryExpander、ExpansionConfig、citation API、chat citations、前端展示  
**变更**：ChatResponse 扩展 citations + rewrite 字段  
**注意**：前端需同步渲染 msg__expansion  
...
| 版本 | 变更 |
|------|------|
| v0.35.0-draft | 仅 QueryExpander |
| v0.35.0 | citation API + chat + 前端 + 20 tests |
...
产品：________  研发：________  测试：________  日期：2026-08-11
"""
...
**验收**：`test_merge_retrieval_results_from_results` 绿；expansion_demo 输出含 source 与 preview。
...
flowchart TD
    ...
    FETCH --> CFG{{ExpansionConfig.enabled?}}
    CFG -->|false| EMPTY[citations=[]]
    CFG -->|true| BUNDLE[ExpandingRetriever.search]
    ...
    CC[ExpansionConfig] --> FETCH
    STORE[(store.json)] --> CC
```

## 2. ExpandingRetriever.search 流程
...
| 组件 | 职责 |
|------|------|
| citation_builder.py | Citation / merge_retrieval_results / CitationBundle |
| context.py | ExpandingRetriever.search |
...
| day36/expansion_demo.py | CLI 引用打印 |
...
flowchart LR
    ...
    RES --> CIT[merge_retrieval_results]
...
## 7. 数据流：PUT expansion-config

1. `ExpansionConfig.from_dict(body)`  
2. `validate()`  
...
```
┌─────────────────────────────────────────────┐
│ Presentation: chat + expansion-config API    │
├─────────────────────────────────────────────┤
│ Application: KnowledgeStore.fetch_citations │
├─────────────────────────────────────────────┤
│ Domain: ExpandingRetriever.search            │
├─────────────────────────────────────────────┤
│ Domain: merge_retrieval_results / CitationBundle    │
├─────────────────────────────────────────────┤
...
{fenced("python", EXPANSION_CONFIG)}
...
{fenced("python", _EXPANSION_CLASS)}
...
{fenced("python", _EXPANDING_SEARCH)}
...
```bash
curl.exe -s http://127.0.0.1:8000/api/knowledge/expansion-config
```
...
3. `test_merge_retrieval_results_from_results_candidates` 验证噪声 chunk 可被翻下去。  
...
**14:05** expansion_demo 输出对比，学员惊呼「source 能对上公告了」。  
**14:50** 讲 `enabled=False` 降级路径。  
**15:15** PUT expansion-config 调 pool=10，延迟表填写。  
**15:58** pytest 第 18 个绿。  
**16:42** `test_merge_retrieval_results_from_results_candidates` 精读。  
...
**Q6** 需求号？ → {REQ}  
**Q7** 平台版本？ → {VER}  
**Q8** 改写类名？ → merge_retrieval_results  
**Q9** 管线类名？ → RAGContextService  
**Q10** inner 类型？ → HybridRetriever  

**Q11** API 路径？ → /api/knowledge/expansion-config  
**Q12** 装配函数？ → _build_rag_service  
**Q13** demo 脚本？ → expansion_demo.py  
**Q14** 测试数？ → 18  
**Q15** 翻牌测试名？ → test_merge_retrieval_results_from_results_candidates  
...
def _file18() -> str:
    return """# Day 35 与 Day 33 能力对照表

| 维度 | Day 33 查询改写 | Day 36 多查询扩展 |
|------|-----------------|-----------------|
| 需求 | ZL-NA-REQ-033 | ZL-NA-REQ-035 |
| 版本 | v0.33.0 | v0.35.0 |
| 核心问题 | 口语问句不规范 | 回答无法追溯来源 |
| 核心模块 | RewritingRetriever | QueryExpander + fetch_citations |
| API 新增 | rewrite-config | expansion-config + citation-preview |
| 存储字段 | rewrite_config | citation_config |
| 测试目录 | tests/day33/ | tests/day35/ |
| demo | rewrite_demo | expansion_demo |
...
```
Day29 Chroma  ──┐
Day30 增量    ├──► Day31 Hybrid ──► Day32 Rerank ──► Day33 Rewrite ──► Day35 Expansion
Day28 rebuild ──┘
```
...
1. 部署 v0.35.0 二进制/容器  
2. 首次启动自动 `ExpansionConfig()` 默认  
3. 跑 `pytest tests/day33 tests/day36`  
4. 抽样 chat citations 回归  
5. 前端确认 msg__expansion 渲染  
...
2. Day34 核心类？ `QueryExpander`  
3. Day34 默认 max_citations？ 3  
4. Day34 API 路径？ expansion-config  
5. 两者串联？ 是（改写后检索，再格式化引用）  
6. Day33 测试数？ 20  
7. Day34 测试数？ 20  
8. 关 citations 字段？ enabled=false  
9. rewrite 元数据？ include_expansion_meta  
10. 下一日？ HyDE 多查询扩展
"""
...
def _file21() -> str:
    return f"""# Day 35 课堂知识竞赛（15 题）

1. 改写器类名？ → `merge_retrieval_results`  
2. 管线类名？ → `RAGContextService`  
3. 默认 max_citations？ → `20`  
4. 默认 enabled？ → `True`  
5. 需求号？ → {REQ}  
6. 平台版本？ → {VER}  
7. expansion-config HTTP 方法？ → GET + PUT  
8. 打分函数？ → `rewrite`  
9. bigram 函数？ → `_bigram_overlap`  
10. inner 检索器？ → `HybridRetriever`  
11. 演示常量文件？ → `day36/constants.py`  
12. 测试总数？ → 18  
13. 精确子串得分？ → 1.0  
14. FR-007 实现？ → `_build_rag_service` 外包  
15. Day35 主题？ → query rewrite（多查询扩展）  
...
**19** 指出 expansion-config 两个路由 HTTP 方法。  
...
def _file22() -> str:
    return f"""# Day 35 精读：query_expander 与多查询扩展管线
...
## 二、行级注释：QueryExpander 抽象（L18–L30）
...
## 三、merge_retrieval_results（L33–L68）
...
## 十、测试精读 test_query_expander.py

{fenced("python", TEST_EXPANDER)}
...
## 十一、API 测试 test_expansion_api.py

{fenced("python", TEST_EXPANSION_API)}
...
读罢 22 精读，你应能**逐行**解释 `merge_retrieval_results` 与 `ExpandingRetriever.search`，并映射到 {REQ} 的 FR-001–FR-003。
...
## 二十六、phase3_expansion_review 建议

课后运行 `src/day35/phase3_expansion_review.py` 串联 Day25–34。
...
`fetch_citations` 是 **Facade**：对外返回 dict，对内调用 ExpandingRetriever.search。与 Day33 Facade 叠加。
...
- RAGContextService + ExpansionConfig
- GET/PUT /api/knowledge/expansion-config
- tests/day36 (20 cases)
```
...
`app.js` 的 `msg__expansion` 渲染 rank/source/score/preview；`msg__rewrite` 展示改写链。
...
for pool in (10, 15, 20, 30, 40):
    store.set_citation_config(ExpansionConfig(enabled=True, max_citations=pool))
    # 对 EXPANSION_QUERIES 打 口语命中 与计时
```
...
`include_expansion_meta=false` 时 citation-preview 的 rewrite 字段应为 null。
...
```python
for n in (1, 2, 3, 5):
    store.set_citation_config(ExpansionConfig(max_citations=n))
...
def _file24() -> str:
    return f"""# Phase 3 第十一日总结（Day 36）
...
| B | 能跑 expansion_demo 解释字段 |
| C | 能复述 citations 与 rewrite 关系 |
| D | 仅会 pytest -q |

## 下周预告

Day 36：HyDE / 多查询扩展 — 一条问句变多条检索 query。
...
{REQ} 已交付：QueryExpander、citation API、chat citations、前端展示、20 测试。下一步：Day35 多 query 扩展。
"""
...
def _file26() -> str:
    return f"""# Day 35 实操 Lab 手册（Lab 0–7）
...
## Lab 2：expansion_demo（25 min）

```bash
python3 src/day35/expansion_demo.py | tee /tmp/day36_demo.txt
```
...
curl -s -X PUT http://127.0.0.1:8000/api/knowledge/expansion-config \\
  -H 'Content-Type: application/json' \\
  -d '{{"enabled":false,"max_citations":3,"preview_max_chars":120,"include_expansion_meta":true}}'
```
...
`lab/day36-<姓名>.md` 含 Lab 4 表格 + Lab 7 截图 + 前端 citations 截图。
...
| # | 测试 | 文件 |
|---|------|------|
| 1–11 | test_query_expander.py | 单元 |
| 12–20 | test_expansion_api.py | API |
